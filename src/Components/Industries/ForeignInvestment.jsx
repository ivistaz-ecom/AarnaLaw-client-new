...
import ForeignInvestmentimg from "../../images/Industries/Foreign-Investment.jpg";
...
function ForeignInvestment() {
  const { selectedLanguage } = useContext(LanguageContext);
  const [selectedIndustry, setSelectedIndustry] = useState(translations[selectedLanguage].Foreign_Investment);
  const [showContactForm, setShowContactForm] = useState(false);
  const location = useLocation();
  const isForeignInvestment = location.search.includes("Industryparam=foreignInvestment");

  useEffect(() => {
    setSelectedIndustry(translations[selectedLanguage].Foreign_Investment);
  }, [selectedLanguage]);
  useEffect(() => {
    window.scrollTo(0, 0);
      ...
      <div className="industry-content">
        <div className="industry-dropdown">
          <IndustryList
            ...
            defaultItem={isForeignInvestment ? translations[selectedLanguage].Foreign_Investment : selectedIndustry}
          />
        </div>

        <div className="industry-description">
          <p>{translations[selectedLanguage].Foreign_Investment_Messpara1}</p>
          <p>{translations[selectedLanguage].Foreign_Investment_Messpara2}</p>
          <p>{translations[selectedLanguage].Foreign_Investment_Messpara3}</p>
        </div>

        <div className="industry-list">
          <ul>
            <li>{translations[selectedLanguage].Foreign_Investment_MessList1}</li>
            <li>{translations[selectedLanguage].Foreign_Investment_MessList2}</li>
            <li>{translations[selectedLanguage].Foreign_Investment_MessList3}</li>
          </ul>
        </div>
